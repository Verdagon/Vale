...
## Why OWPFRD compares against `denizenTemplateId`, not `id`

**Alternative considered:** convert `rootDenizenEnv.id` to a template name by pattern-matching on the `localName` (extracting `FunctionTemplateNameT` from `FunctionNameT`, etc.) and keeping `initSteps`.

**Why rejected:** this was the previous approach and it's subtly wrong for lifted functions. See @PNBDTZ. The issue is that for a lifted function like `drop(self Ok<T,E>)`, `id.initSteps` contains the *instantiated* parent citizen (`Ok<T_placeholder, E_placeholder>`) while the placeholder's name prefix is rooted at the *pre-lift* function template (`[drop]`, a single step with empty initSteps). The post-lift id can never be a prefix of the pre-lift placeholder namespace because the post-lift id is *longer* — the template conversion only rewrites `localName`, not `initSteps`.

**Chosen approach:** use `rootDenizenEnv.denizenTemplateId` directly. This is the pre-lift template ID — for the `drop` example, it's `IdT(pkg, [], FunctionTemplateNameT(drop))`, matching how placeholders were named at creation.

**Why this is robust:** `denizenTemplateId` is an invariant set when the denizen env is constructed and unchanged by lifting. Any placeholder-naming code path that uses the same value will agree with the sanity check. The previous code only worked for non-lifted functions by accident — it was fragile.
