...
And one binary from a separate repo:

| Binary | Where to get it |
|---|---|
| `Divination` | Clone `/Volumes/V/Divination` (or wherever the repo lives) and `cargo build`. Binary lands at `target/debug/Divination`. |
...
After the prerequisites are met, the smoke test in this repo proves the whole pipeline end-to-end:

```
DIVINATION_PATH=/path/to/Divination/target/debug/Divination \
RUST_CARGO_TOML=/path/to/your/Dependencies.toml \
./scripts/test-rust-interop.sh
```

It writes a minimal program at `/tmp/vale_rust_interop_test/src/main.vale`, builds it, runs the binary, and asserts exit code 42. If that passes, your environment is wired correctly.
