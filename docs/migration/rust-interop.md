...
## Divination has no `rust-toolchain.toml`

The `rust-toolchain.toml` at this repo's root pins the toolchain for everything inside `ValeRustInterop/` (ValeRuster, FrontendRust, CoordinatorRust). But Divination lives in a separate repo and inherits whatever toolchain the user happens to have active — which can drift from what the pipeline expects.

**Resolution:** add a matching `rust-toolchain.toml` to Divination's repo root. Cheap (copy-paste the file), just not done yet.
...
**Resolution:** either document the requirement (done in `docs/usage/rust-interop.md` and `scripts/test-rust-interop.sh`), make the Coordinator default `--clang_override` to `/usr/bin/clang` on macOS, or detect the swiftly shim and warn. Any of these is small; we did the docs version.
