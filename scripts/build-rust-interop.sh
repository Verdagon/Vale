...
# Builds the four in-repo components (Frontend, Backend, ValeRuster, Coordinator) and optionally
# Divination if DIVINATION_DIR is set (Divination lives in a separate repo).
#
# Usage:
...
#   DIVINATION_DIR      If set, `cargo build` is run there. Divination generates the Rust crate
#                       that wraps imported Rust types; the Backend invokes it at runtime.
...
# Divination: external repo, only built if DIVINATION_DIR is provided. The runtime pipeline
# needs it to size/link Rust types, but it's not part of this repo so building here is optional.
if [ -n "${DIVINATION_DIR:-}" ] && [ -z "${SKIP_DIVINATION:-}" ]; then
  echo ""
...
[ -n "${DIVINATION_DIR:-}" ] && [ -z "${SKIP_DIVINATION:-}" ] && echo "  Divination:  $DIVINATION_DIR/target/debug/Divination"
echo ""
echo "Run ./scripts/test-rust-interop.sh to smoke-test the pipeline."
