...
class Lexer(interner: Interner, keywords: Keywords) {
      ...
      Ok(
        Some(
          MacroCallL(
            ...
            DontCallMacroL,
            WordLE(RangeL(attributeBegin, end), keywords.DeriveStructDrop))))
    } else if (iter.trySkipCompleteWord("#DeriveStructConstructor")) { // DO NOT SUBMIT seems needlessly repetitive
      val end = iter.getPos()
      Ok(
        Some(
          MacroCallL(
  ...
  Result[Option[StructL], IParseError] = {
    ...
    // with us lexing functions outside too. Internal methods (struct body containing
    // `func`/`extern func` declarations) are pre-extracted into `methodsAcc`; remaining
    // member declarations are lexed as scrambles.
