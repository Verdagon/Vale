...
class IfTests extends FunSuite with Matchers with Collector with TestParseUtils {
  ...
  test("if let") {
    compileExpressionExpect("if [u] = a {}") shouldHave {
      case IfPE(_,
        LetPE(_,
          PatternPP(_,None,None,None,None,
            Some(
              DestructureP(_,
                Vector(
                  PatternPP(_,None,Some(LocalNameDeclarationP(NameP(_, StrI("u")))),None,None,None,None)))),
            None),
  ...
  test("If with condition declarations") {
    compileExpressionExpect("if x = 4; not x.isEmpty() { }") shouldHave {
      case IfPE(_,
        ConsecutorPE(
          Vector(
            LetPE(_,PatternPP(_,None,Some(LocalNameDeclarationP(NameP(_, StrI("x")))),None,None,None,None),ConstantIntPE(_,4,None)),
  ...
  test("19") {
    compileBlockContentsExpect(
      ...
      case ConsecutorPE(
        Vector(
          LetPE(_,
            PatternPP(_,_,Some(LocalNameDeclarationP(NameP(_, StrI("newLen")))),None,None,None,None),
            IfPE(_,
