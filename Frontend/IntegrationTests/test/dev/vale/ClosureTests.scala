...
//        _) =>
  ...
  test("Mutates from inside a closure") {
    ...
    Collector.only(lambda, {
      case MutateTE(
        AddressMemberLookupTE(_,_,IdT(_, Vector(FunctionNameT(FunctionTemplateNameT(StrI("main"), _),Vector(),Vector()), LambdaCitizenNameT(_)),CodeVarNameT(StrI("x"))),CoordT(ShareT,_,IntT.i32), _),
        _) =>
    })
