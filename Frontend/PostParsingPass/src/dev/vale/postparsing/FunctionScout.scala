              ...
              case (None, Some(patternP)) => {
                ...
                patternPerhapsWithoutCoordRuneS.coordRune match {
                  case None => {
                    ...
                    // genericParams and to the identifying runes. Per @LAGTNGZ, this keeps the
                    // `_` vs `<T>` distinction contained in the postparser so later passes see
                    // a uniform FunctionS shape. This only happens for lambdas, top level
                    // functions can't have these, enforced elsewhere.
                    val rune = rules.RuneUsage(rangeS, ImplicitRuneS(lidb.child().consume()))
                    runeToExplicitType += ((rune.rune, CoordTemplataType()))
