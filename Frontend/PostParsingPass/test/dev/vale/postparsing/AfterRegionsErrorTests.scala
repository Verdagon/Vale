...
class AfterRegionsErrorTests extends FunSuite with Matchers with Collector {
  ...
  test("Test one-anonymous-param lambda identifying runes") {
    ...
    // Per @LAGTNGZ, the postparser publishes one GenericParameterS per untyped lambda
    // param — so later passes see a uniform FunctionS regardless of whether the user
    // wrote `<T>` or `(_)`. LAGTNGZ still governs how the typing pass specializes
    // the lambda (per-call template expansion into LambdaCallFunctionTemplateNameT).
