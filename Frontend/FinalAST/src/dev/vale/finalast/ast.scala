...
//    immDestructorsByKind: Map[KindH, PrototypeH],
  ...
  def lookupFunction(readableName: String) = {
    val matches =
      (Vector.empty ++
        ...
        functions.filter(_.prototype.id.readableName == readableName).map(_.prototype))
        .distinct
  ...
  def lookupStruct(humanName: String) = {
    val matches = structs.filter(_.id.readableName == humanName)
    vassert(matches.size == 1)
    matches.head
  }
  ...
  def lookupInterface(humanName: String) = {
    val matches = interfaces.filter(_.id.readableName == humanName)
    vassert(matches.size == 1)
    matches.head
  }
}
