...
import dev.vale.typing.env.{AddressibleClosureVariableT, AddressibleLocalVariableT, ExpressionLookupContext, FunctionEnvironmentT, IInDenizenEnvironmentT, ILocalVariableT, NodeEnvironmentBox, NodeEnvironmentT, ReferenceClosureVariableT, ReferenceLocalVariableT, TemplataEnvEntry, TemplataLookupContext}
import dev.vale.typing.function.{DestructorCompiler, IEvaluateFunctionResult}
          ...
          val templataFromEnv =
            nenv.lookupAllWithImpreciseName(name, Set(ExpressionLookupContext)) match {
              ...
              case templatas if templatas.nonEmpty &&
                templatas.collect({ case FunctionTemplataT(
                _,
                _) =>
                case ExternFunctionTemplataT(_) =>
                }).size == templatas.size => {
        ...
        case IndexSE(range, containerExpr1, indexExpr1) => {
          ...
          val exprTemplata =
            containerExpr2.result.coord.kind match {
              ...
              //              case at@StructTT(IdT(ProgramT.topLevelName, Vector(),
...
//          vassert(!thenBodySE.pure)
              ...
              .copy(templatas =
                nenv.snapshot.templatas
                  .addEntry(
                    ...
                    TemplataEnvEntry(templata.CoordTemplataT(elementRefT))))
          val makeListTE =
            callCompiler.evaluatePrefixCall(
              coutputs,
              nenv,
            ...
            TemplataCompiler.getFunctionTemplate(nenv.snapshot.id)
              .addStep(
                interner.intern(RegionPlaceholderNameT(
                  ...
                  ReadWriteRegionS)))
          val newRegion = RegionT(PlaceholderTemplataT(newRegionId, RegionTemplataType()))
  ...
  (ReferenceExpressionTE) = {
    ...
    //    val closureVarId = Id2(nenv.lambdaNumber, "__closure_" + function1.origin
    //    .lambdaNumber)
