...
LLVMValueRef declareFunction(
  ...
  auto returnTypeL =
      region->translateType(globalState, functionM->prototype->returnType);
...
void translateFunction(
  ...
  auto returnTypeL = defaultRegion->translateType(globalState, functionM->prototype->returnType);
  ...
  if (LLVMTypeOf(resultLE) == makeNeverType()) {
    LLVMBuildRet(bodyTopLevelBuilder, LLVMGetUndef(defaultRegion->translateType(globalState, functionM->prototype->returnType)));
  }
