  ...
  buildIfNever(
      ...
      [globalState, failMessage](LLVMBuilderRef bodyBuilder) {
        if (globalState->opt->includeBoundsChecks) {
          ...
          buildSimpleCall(bodyBuilder, globalState->externs->exit, {constI64LE(globalState, 1)});
        }
...
Ref swapElement(
  ...
  storeInnerArrayMember(globalState, functionState, builder, arrayElemsPtrLE, indexLE, sourceLE);
...
//
//  auto indexLE =
//      adjustCounterReturnOld(globalState, builder, globalState->metalCache->i32, sizePtrLE, 1);
//  // Manually making this token because we know it's in bounds because we just incremented size.
//  auto indexInBoundsLE = InBoundsLE{indexLE};
//
//  auto sourceLE =
//      globalState->getRegion(elementRefM)
//          ->checkValidReference(FL(), functionState, builder, false, elementRefM, sourceRef);
//  storeInnerArrayMember(globalState, functionState, builder, elemsPtrLE, indexInBoundsLE, sourceLE);
//}
