...
//  def lookupTemplata(profiler: IProfiler, env: Env, range: RangeS, name: INameS): ITemplata
//}
//
...
//          return (InferMatchConflict(inferences.inferences, range, "Rune " + rune + " is of type " + expectedType + ", but it received a " + templataFromEnv.tyype + ", specifically " + templataFromEnv, Vector.empty))
//        }
//        templataFromEnv
//      }
//    val equal =
...
//          matchParamFilterAgainstAtomSP(env, state, runeToType, localRunes, inferences, paramFilter, part) match {
...
//      matchReference2AgainstRuneSP(env, state, runeToType, localRunes, inferences, patternRange, instance.tyype, NameTranslator.translateRune(runeCoordRuneA)) match {
//        case imc @ InferMatchConflict(_, _, _, _) => return imc
...
//        case None => (true)
//        case Some(parts) => {
//          matchReference2AgainstDestructure(env, state, patternRange, runeToType, localRunes, inferences, instance.tyype, parts) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
//            case (InferMatchSuccess(ds)) => (ds)
//          }
//        }
...
//          matchKind2AgainstRuneSP(env, state, runeToType, localRunes, inferences, range, instanceSuperInterfaceRef2, NameTranslator.translateRune(kindRuneA)) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
//            case (InferMatchSuccess(ds)) => (ds)
//          }
//        }
...
//  IInferMatchResult = {
//    evaluate(env, state, runeToType, localRunes, inferences, call.template) match {
...
//        (InferMatchConflict(inferences.inferences, range, "Couldn't figure out template!", Vector.empty))
//      }
...
//                matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, actualArg, expectedArg) match {
//                  case imc @ InferMatchConflict(_, _, _, _) => return imc
//                  case InferMatchSuccess(deeplySatisfied) => (deeplySatisfiedSoFar && deeplySatisfied)
//                }
//              }
...
//      matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, ArrayTemplateTemplata(), expectedTemplate) match {
//        case imc @ InferMatchConflict(_, _, _, _) => return imc
//        case (InferMatchSuccess(ds)) => (ds)
//      }
//    // Check to see that the actual template args match the expected template args
...
//      return InferMatchConflict(inferences.inferences, range, "Supplied wrong number of template arguments!", Vector.empty)
//    }
//    val argsDeeplySatisfied =
//      expectedArgs.zip(actualArgs).foldLeft((true))({
//        case ((deeplySatisfiedSoFar), (expectedArg, actualArg)) => {
...
//      matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, MutabilityTemplata(mutability), mutabilityTemplex) match {
//        case (imc@InferMatchConflict(_, _, _, _)) => return imc
//        case InferMatchSuccess(deeplySatisfied) => (deeplySatisfied)
//      }
//
//    val variabilityDeeplySatisfied =
//      matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, VariabilityTemplata(variability), variabilityTemplex) match {
//        case (imc@InferMatchConflict(_, _, _, _)) => return imc
//        case InferMatchSuccess(deeplySatisfied) => (deeplySatisfied)
//      }
//
//    val sizeDeeplySatisfied =
//      matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, IntegerTemplata(size), sizeTemplex) match {
//        case (imc@InferMatchConflict(_, _, _, _)) => return imc
//        case InferMatchSuccess(deeplySatisfied) => (deeplySatisfied)
//      }
//
//    val elementDeeplySatisfied =
//      matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, CoordTemplata(elementCoord), elementTemplex) match {
//        case (imc@InferMatchConflict(_, _, _, _)) => return imc
//        case InferMatchSuccess(deeplySatisfied) => (deeplySatisfied)
//      }
//
...
//        case ((deeplySatisfiedSoFar), (expectedArg, actualArg)) => {
//          matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, CoordTemplata(actualArg), expectedArg) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
//            case InferMatchSuccess(deeplySatisfied) => (deeplySatisfiedSoFar && deeplySatisfied)
//          }
//        }
...
//        matchTemplataAgainstEqualsTR(env, state, runeToType, localRunes, inferences, instance, rule)
//      }
//      case rule @ IsaTR(_, _, _) => {
//        matchTemplataAgainstIsaTR(env, state, runeToType, localRunes, inferences, instance, rule)
//      }
//      case rule @ OrTR(_, _) => {
//        matchTemplataAgainstOrTR(env, state, runeToType, localRunes, inferences, instance, rule)
//      }
//      case rule @ ComponentsTR(_, _, _) => {
//        matchTemplataAgainstComponentsTR(env, state, runeToType, localRunes, inferences, instance, rule)
//      }
//      case rule @ CallTR(_, _, _, _) => {
//        matchTemplataAgainstCallTR(env, state, runeToType, localRunes, inferences, instance, rule)
//      }
//      case _ => {
...
//            runeToType.get(rune) match {
//              case None =>
//              case Some(expectedTypeFromAbove) => vassert(expectedType == expectedTypeFromAbove)
//            }
//            matchTemplataAgainstRuneSP(env, state, runeToType, localRunes, inferences, range, actualTemplata, rune, expectedType) match {
//              case imc @ InferMatchConflict(_, _, _, _) => return imc
...
//            //          evaluate(env, state, runeToType, localRunes, inferences, TemplexTR(ct)) match {
...
//
//            matchCitizenAgainstCallTT(env, state, runeToType, localRunes, inferences, range, ct, structTT)
//            //        }
//
//
...
//            }
//
//            matchCitizenAgainstCallTT(env, state, runeToType, localRunes, inferences, range, ct, cit)
//          }
...
//              env, state, runeToType, localRunes, inferences, range, expectedTemplate, expectedArgs, Vector(MutabilityTemplata(mutability), VariabilityTemplata(variability), CoordTemplata(elementArg)))
//          }
...
//          //              matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, CoordTemplata(actualMember), expectedMember) match {
//          //                case imc @ InferMatchConflict(_, _, _, _) => return imc
//          //                case InferMatchSuccess(deeplySatisfied) => (deeplySatisfiedSoFar && deeplySatisfied)
//          //              }
//          //            }
...
//            matchStaticSizedArrayKind(env, state, runeToType, localRunes, inferences, mutabilityTemplex, variabilityTemplex, sizeTemplex, elementTemplex, size, elementCoord, mutability, variability)
//          }
...
//            matchTupleKind(env, state, runeToType, localRunes, inferences, expectedElementTemplexesT, elements)
//          }
...
//                matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, CoordTemplata(actualCoord), expectedCoordRule) match {
...
//              }
//            matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, resultCoord, innerCoordTemplex)
//          }
//        }
//      }
//    }
...
//      return (InferMatchConflict(inferences.inferences, range, "Call result expected type " + resultType + ", but was " + instance, Vector.empty))
//    }
//
//    name match {
//      case "toRef" => {
//        val Vector(kindRule) = args
//        instance match {
...
//              return (InferMatchConflict(inferences.inferences, range, "Coord matching into toRef doesn't have default ownership: " + instanceOwnership, Vector.empty))
//            }
//            matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, KindTemplata(instanceKind), kindRule)
//          }
//          case _ => return (InferMatchConflict(inferences.inferences, range, "Bad arguments to toRef: " + args, Vector.empty))
//        }
//      }
...
//            matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, instance, kindRule)
//          }
//          case _ => return (InferMatchConflict(inferences.inferences, range, "Bad arguments to passThroughIfConcrete: " + args, Vector.empty))
//        }
//      }
...
//            matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, instance, kindRule)
//          }
//          case _ => return (InferMatchConflict(inferences.inferences, range, "Bad arguments to passThroughIfInterface: " + args, Vector.empty))
//        }
//      }
...
//            matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, instance, kindRule)
//          }
//          case _ => return (InferMatchConflict(inferences.inferences, range, "Bad arguments to passThroughIfStruct: " + instance, Vector.empty))
//        }
//      }
//    }
//  }
...
//      return (InferMatchConflict(inferences.inferences, range, s"Supplied templata isn't the right type! Type: ${rule.tyype} but gave: ${instance}", Vector.empty))
//    }
//
//    instance match {
...
//          env, state, runeToType, localRunes, inferences, MutabilityTemplata(actualMutability), mutabilityRule)
//      }
//      case CoordTemplata(actualReference) => {
//        vcheck(components.size == 3, "Wrong number of components for coord")
//        val Vector(ownershipRule, permissionRule, kindRule) = components
//        val actualOwnership = OwnershipTemplata(actualReference.ownership)
//        val ownershipDeeplySatisfied =
//          matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, actualOwnership, ownershipRule) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
...
//          matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, actualPermission, permissionRule) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
//            case (InferMatchSuccess(ods)) => (ods)
//          }
//        val actualKind = KindTemplata(actualReference.kind)
//        val kindDeeplySatisfied =
//          matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, actualKind, kindRule) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
...
//        val Vector(nameRule, paramsRule, retRule) = components
//        val actualHumanNameTemplata =
//          actualPrototype.fullName.last match {
//            case FunctionNameT(humanName, _, _) => StringTemplata(humanName)
//            case _ => return InferMatchConflict(inferences.inferences, range, "Actual prototype doesn't have a human name: " + actualPrototype.fullName.last, Vector.empty)
//          }
...
//          matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, actualHumanNameTemplata, nameRule) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
//            case (InferMatchSuccess(ods)) => ods
//          }
//
//        val paramsDeeplySatisfied =
//          matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, actualParamsTemplata, paramsRule) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
//            case (InferMatchSuccess(kds)) => (kds)
//          }
//
//        val retDeeplySatisfied =
//          matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, actualRetTemplata, retRule) match {
//            case imc @ InferMatchConflict(_, _, _, _) => return imc
//            case (InferMatchSuccess(kds)) => (kds)
//          }
//
...
//
//    matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, instance, left) match {
//      case imc @ InferMatchConflict(_, _, _, _) => imc
//      case (InferMatchSuccess(leftDeeplySatisfied)) => {
//        matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, instance, right) match {
//          case imc @ InferMatchConflict(_, _, _, _) => imc
...
//
//    matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, subTemplata, left) match {
//      case imc @ InferMatchConflict(_, _, _, _) => imc
//      case (InferMatchSuccess(subDeeplySatisfied)) => {
//        evaluate(env, state, runeToType, localRunes, inferences, right) match {
//          case (iec @ InferEvaluateConflict(_, _, _, _)) => return (InferMatchConflict(inferences.inferences, range, "Couldn't evaluate concept!", Vector(iec)))
...
//
//    val results = possibilities.map(matchTemplataAgainstRulexTR(env, state, runeToType, localRunes, inferences, instance, _))
//
