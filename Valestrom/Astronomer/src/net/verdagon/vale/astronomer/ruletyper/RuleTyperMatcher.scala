...
//      case (StringSR(range, value), StringTemplataType) => (RuleTyperMatchSuccess(StringAR(range, value)))
//      case (MutabilitySR(range, value), MutabilityTemplataType) => (RuleTyperMatchSuccess(MutabilityAR(range, value)))
//      case (PermissionSR(range, value), PermissionTemplataType) => (RuleTyperMatchSuccess(PermissionAR(range, value)))
//      case (LocationSR(range, value), LocationTemplataType) => (RuleTyperMatchSuccess(LocationAR(range, value)))
//      case (OwnershipSR(range, value), OwnershipTemplataType) => (RuleTyperMatchSuccess(OwnershipAR(range, value)))
//      case (VariabilitySR(range, value), VariabilityTemplataType) => (RuleTyperMatchSuccess(VariabilityAR(range, value)))
//      case (AbsoluteNameSR(range, nameS), _) => {
...
//          case RuleTyperMatchSuccess(()) => RuleTyperMatchSuccess(AbsoluteNameAR(range, nameA, expectedType))
...
//        }
//      }
//      case (NameSR(range, nameS), _) => {
//        val tyype = delegate.lookupType(state, env, range, nameS)
//        val nameA = Astronomer.translateImpreciseName(nameS)
//        matchNameTypeAgainstTemplataType(conclusions, range, tyype, expectedType) match {
//          case RuleTyperMatchSuccess(()) => RuleTyperMatchSuccess(NameAR(range, nameA, expectedType))
//          case rtmc @ RuleTyperMatchConflict(_, _, _, _) => {
//            return (RuleTyperMatchConflict(conclusions.conclusions, range, nameA + "doesn't match needed " + expectedType, Vector(rtmc)))
//          }
//        }
//      }
//      case (RuneSR(range, runeS), _) => {
...
//          case (RuleTyperMatchSuccess(())) => {
//            (RuleTyperMatchSuccess(RuneAR(range, runeA, expectedType)))
//          }
//        }
//      }
...
////        vfail("impl?")
////      }
//      case (CallSR(range, template, templateArgs), KindTemplataType | CoordTemplataType) => {
...
//        }
//      }
//      case (PackSR(range, elements), PackTemplataType(CoordTemplataType)) => {
...
////        }
////      }
//      case (InterpretedSR(range, ownership, permission, coordTemplex), CoordTemplataType) => {
...
//        }
//      }
//      case (RepeaterSequenceSR(range, mutabilityTemplexS, variabilityTemplexS, sizeTemplexS, elementTemplexS), KindTemplataType | CoordTemplataType) => {
...
//        }
//      }
//      case (ManualSequenceSR(range, elementTemplexesS), KindTemplataType | CoordTemplataType) => {
//        val templexMaybesA =
...
//        }
//      }
//      case (BorrowSR(range, inner), CoordTemplataType) => {
...
//      //            } else {
//      //              RuleTyperMatchSuccess(NameAR(name, expectedType))
//      //            }
//      //          }
//      //          // Is this right? Can't we look it up as a coord, like we did with KindTemplata/CoordTemplataType?
...
//    }
//
//    RuleTyperMatchSuccess(TemplexAR(RuneAR(range, runeA, expectedType)))
//  }
//
